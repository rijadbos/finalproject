...
#define ROWS 4
#define COLS 4
int scanRowPins[ROWS] = {37,36,35,0}; // Keypad row pins
int colPins[COLS]     = {45,48,47,21};
char keys[ROWS][COLS] = {
{'1','2','3','A'},
{'4','5','6','B'},
{'7','8','9','C'},
...
// Only return when a new key is pressed
if(k != '\0' && k != last_key){
last_key = k;
return k;
}
...
i2c_config_t conf = {.mode=I2C_MODE_MASTER, .sda_io_num=sda,.scl_io_num=scl,
...
if (nibble & 4)  { out |= (1 << 6); }
if (nibble & 8)  { out |= (1 << 7); }
if(flags&PCF_RS) out|=PCF_RS;
pcf_write_one(g_pcf_addr,out|PCF_EN); vTaskDelay(pdMS_TO_TICKS(1));
pcf_write_one(g_pcf_addr,out & ~PCF_EN); vTaskDelay(pdMS_TO_TICKS(1));
}
...
static void lcd_putc(char c){ lcd_send_byte(c,true); }
...
lcd_clear(); lcd_set_cursor(0,0); for(int i=0;l1 && l1[i] && i<16;i++) lcd_putc(l1[i]);
lcd_set_cursor(0,1); for(int i=0;l2 && l2[i] && i<16;i++) lcd_putc(l2[i]);
}
static void lcd_init(){
vTaskDelay(pdMS_TO_TICKS(50)); pcf_write_one(g_pcf_addr,0); vTaskDelay(pdMS_TO_TICKS(5));
pcf_write_nibble(0x03,0); vTaskDelay(pdMS_TO_TICKS(5)); pcf_write_nibble(0x03,0); vTaskDelay(pdMS_TO_TICKS(5));
pcf_write_nibble(0x03,0); vTaskDelay(pdMS_TO_TICKS(5)); pcf_write_nibble(0x02,0); vTaskDelay(pdMS_TO_TICKS(5));
lcd_cmd(0x28); lcd_cmd(0x0C); lcd_cmd(0x01); vTaskDelay(pdMS_TO_TICKS(2)); lcd_cmd(0x06);
}

/* ============================================================
MPU6050 FUNCTIONS
============================================================ */
static esp_err_t mpu_wake(){ uint8_t cmd[2]={MPU6050_PWR_MGMT_1,0x00}; return i2c_master_write_to_device(MPU_I2C_NUM,MPU6050_ADDR,cmd,2,pdMS_TO_TICKS(200)); }
static esp_err_t mpu_read_raw(float *ax,float *ay,float *az,float *gx,float *gy,float *gz,float *temp){
uint8_t buf[14]; uint8_t reg=MPU6050_ACCEL_XOUT_H;
esp_err_t r=i2c_master_write_read_device(MPU_I2C_NUM,MPU6050_ADDR,&reg,1,buf,sizeof(buf),pdMS_TO_TICKS(200));
if(r!=ESP_OK) return r;
int16_t axr=(buf[0]<<8)|buf[1]; int16_t ayr=(buf[2]<<8)|buf[3]; int16_t azr=(buf[4]<<8)|buf[5]; int16_t gxr=(buf[8]<<8)|buf[9]; int16_t gyr=(buf[10]<<8)|buf[11]; int16_t gzr=(buf[12]<<8)|buf[13]; int16_t tr=(buf[6]<<8)|buf[7];
*ax=axr/16384.0f; *ay=ayr/16384.0f; *az=azr/16384.0f; *gx=gxr/16384.0f; *gy=gyr/16384.0f; *gz=gzr/16384.0f;  *temp=tr/340.0f+36.53f;
return ESP_OK;
}
...
void app_main() {
// Initialize LEDs
gpio_set_direction(GREEN_LED,GPIO_MODE_OUTPUT); gpio_set_level(GREEN_LED,0);
gpio_set_direction(RED_LED,GPIO_MODE_OUTPUT); gpio_set_level(RED_LED,0);
    ...
    if(k!='\0' && state==STATE_POWERUP){
        ...
        if(k=='#'){ // submit
            if(strcmp(entered,CORRECT_PIN)==0){
                ...
                gpio_set_level(GREEN_LED,1); vTaskDelay(pdMS_TO_TICKS(5000)); gpio_set_level(GREEN_LED,0);
            }else{
                state=STATE_DENIED;
                if(g_has_pcf) lcd_print_lines("Access Denied","");
                gpio_set_level(RED_LED,1); vTaskDelay(pdMS_TO_TICKS(2000)); gpio_set_level(RED_LED,0);
                memset(entered,0,5); pos=0; state=STATE_POWERUP;
            }
        }
        if(k=='*'){ pos=0; memset(entered,0,5); if(g_has_pcf) lcd_print_lines("Enter Key Code:",""); }
    }
